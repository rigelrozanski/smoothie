...
func NewLine(start, end Point) Line {
	...
	if denom.Equal(zero) {
		return Line{start, end, ZeroDec(), start.Y}
	}
...
func (l Line) Intercept(l2 Line) (intercept Point, withinBounds, isStartingPt, isEndingPt bool) {

	// same line characteristics - take the nearest point
	if ((l.M.Sub(l2.M)).Abs()).LT(precErr) &&
		((l.B.Sub(l2.B)).Abs()).LT(precErr) {

		fmt.Println("hitIntercept")

		x := MinDec(l.End.X, l2.End.X)
		if x.Equal(l.Start.X) { // sanity
			panic("why")
		}
		intercept = l.PointWithX(x)
		fmt.Printf("debug intercept: %v\n", intercept)
		return intercept, true, false, true
	}

	// lines are completly horizontal or flat
	...
	if l.Start.X.Equal(l2.Start.X) && l.Start.Y.Equal(l2.Start.Y) {
		return l.Start, false, true, false
	}
	...
	if lXLine {
		...
		denom := l.M.Sub(l2.M)
		if denom.Equal(zero) {
			return intercept, false, false, false
		}
		y := num.Quo(denom)
